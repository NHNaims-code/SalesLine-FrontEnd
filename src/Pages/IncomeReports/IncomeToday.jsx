...
export default function IncomeToday() {
  ...
  const handleDelete = async(rowData) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
          ...
          Delete(`payment/deletepayment/${rowData._id}`).then(response => {
            if(!response.data == false){                
              const todaysData = [];
  ...
  return (
    <div>
      <DataTable loading={loading} tableTitle="Today Income" columns={columns} rows={allpayment} handleEdit={(rowData) => {setUpdateData(rowData); setOpen(true)}} handleDelete={handleDelete}/>
